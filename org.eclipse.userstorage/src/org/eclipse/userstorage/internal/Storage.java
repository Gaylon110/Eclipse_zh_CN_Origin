...
public final class Storage implements IStorage
{
  ...
  public List<IBlob> getBlobs(int pageSize, int page) throws IOException, NotFoundException
  {
    ...
    Map<String, Map<String, Object>> properties = session.retrieveProperties(applicationToken, pageSize, page);
  ...
  public InputStream retrieveBlob(String key, Map<String, String> properties) throws IOException, NoServiceException
  {
    ...
    {
      Session session = getSession();
      InputStream contents = session.retrieveBlob(applicationToken, key, properties, cacheStream != null);
  ...
  public boolean updateBlob(String key, Map<String, String> properties, InputStream in) throws IOException, NoServiceException, ConflictException
  {
    ...
    try
    {
      created = session.updateBlob(applicationToken, key, properties, in);
    }
  ...
  {
    Session session = getSession();
    boolean deleted = session.deleteBlob(applicationToken, key, properties);
  ...
  public Session getSession() throws NoServiceException
  {
    ...
    ICredentialsProvider provider = credentialsProvider;
    if (provider == null)
    {
      provider = getServiceSafe().getCredentialsProvider();
    }
    session = ((AbstractCredentialsProvider)provider).openSession(getServiceSafe());
    return session;
  }
  ...
  public Connectedness getConnectedness()
  {
    ...
    {
      IStorageService service = getService();
      if (service != null && credentialsProvider.hasCredentials(service))
      {
        return Connectedness.AUTHORIZED;
      }
    }
    return Connectedness.UNAUTHORIZED;
  }
