...
public final class ColumnsAddService {
	...
	public ColumnsAddService(final Table table,
			final BasicCommandFactory basicCmdFactory) {
		this.table = table;
		this.facetCustomCmdF = new FacetAndCustomCommandFactory(table,
				basicCmdFactory);
	...
	public IETypedElementSelectionDialog<Object> openAddColumnsDialog() {
		...
		final List<FacetSet> knownFacetSets = CustomizationUtils
				.getKnownFacetSetsWithoutCustom(this.table);
		...
		final IDialogCallbackWithPreCommit<List<ETypedElement>, Object, Object> callback = new IDialogCallbackWithPreCommit<List<ETypedElement>, Object, Object>() {
			...
			public void committed(final List<ETypedElement> result,
				...
				for (ETypedElement eTypedElement : result) {
					final FacetSet facetSet = FacetUtils
							.getFacetSet(eTypedElement);
					if (facetSet != null) {
						final FacetSet rootFacetSet = FacetUtils
								.getRootFacetSet(facetSet);
				...
				addColumns(resolvedTE, new ArrayList<FacetSet>(facetSets));
			}
		...
		final IETypedElementSelectionDialog<Object> dialog = IETypedElementSelectionDialogFactory.DEFAULT
				.openETypedElementSelectionDialog(eTypedElements,
						Integer.MAX_VALUE, false, callback,
						Display.getDefault().getActiveShell(), customManager,
						knownFacetSets);
		return dialog;
	}

	public void addColumns(final List<ETypedElement> eTypedElements,
			final List<FacetSet> facetSets) {
		final Command signatureFSetCmd = this.facetCustomCmdF
				.createSignatureFacetSetCommand(eTypedElements, facetSets);
		final Command cmd = this.columnsCmdFactory
				.createAddColumnCommand(eTypedElements, signatureFSetCmd);
