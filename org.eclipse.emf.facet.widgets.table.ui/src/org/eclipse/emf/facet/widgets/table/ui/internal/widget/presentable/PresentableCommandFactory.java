...
public class PresentableCommandFactory {
	...
	public PresentableCommandFactory(final Table table,
			final BasicCommandFactory basicCmdFactory,
			final TableCommandFactory cmdFactory) {
		this.table = table;
		this.basicCmdFactory = basicCmdFactory;
		this.tableCmdFactory = cmdFactory;
	}

	public Command createSetCanBePresentedInTheTableCommand(
		...
		if (removeNonConform) {
			final Command command = createRemoveNonConformingElementsCommand(
					presentableQuery);
			if (command != null) {
		...
		compoundCommand
				.setLabel(Messages.NatTableWidget_setPresentabilityQuery);
	...
	public boolean acceptsElement(final Object element,
		...
		if (presentableQuery == null) {
			...
			try {
				result = facetManager
						.getOrInvoke(eObject, presentableQuery, Boolean.class)
						.booleanValue();
			} catch (Exception e) {
				Logger.logError(e,
						"Error executing presentability query: " //$NON-NLS-1$
								+ presentableQuery.getName(),
