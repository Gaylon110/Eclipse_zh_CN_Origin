...
public class ColumnsHideService {
	...
	public ColumnsHideService(final Table table,
	...
	public IColumnsToHideDialog openColumnsToHideDialog() {
		...
		final boolean localCustoMustTop = PreferenceConstants
				.isMustPutOnTheTopTheLocalCustom();
		final boolean askTopLocalCusto = PreferenceConstants
				.isAskToPutOnTheTopTheLocalCustomization();
		final ColumnsToHideDialog dialog = new ColumnsToHideDialog(
				Display.getDefault().getActiveShell(), this.table.getColumns(),
				labelProvider,
				CustomizationUtils.isOneOfTheLocalCustomizationsNotOnTheTop(
						this.table) && askTopLocalCusto,
				localCustoMustTop) {
			@Override
			protected void okPressed() {
				super.okPressed();
				if (getResult() != null) {
					final boolean putOnTop = isPutOnTheTop()
							&& CustomizationUtils
									.isOneOfTheLocalCustomizationsNotOnTheTop(
											getTable());
					showHideColumns(getSelectedColumns(), putOnTop);
					setMustAskThUserToPutOnTheTopTheLocalCustomizationNextTime(
	...
	public void showHideColumns(final List<Column> columnToShow,
			final boolean putOnTheTop) {
		final List<Column> visibleColumns = getVisibleColumns(putOnTheTop);
		final Command cmd = this.columnsCmdFactory.createShowHideColumnCommand(
				columnToShow, visibleColumns, putOnTheTop);
	 ...
	 *            a table
	 * @param satAtTheTop
	 *            if(<code>true</code>, we calculate the future state of the
	 *            localCustom
	 * @return all the columns which are visible, using the local customization
	 *         file
	 * @throws CustomizationException
	 */
	public List<Column> getVisibleColumnsUsingTheLocalCustomizations(
			final boolean setToTheTop) throws CustomizationException {
		...
		if (setToTheTop) {
			...
			for (Customization customization : this.table
					.getLocalCustomizations()) {
				customManager.getManagedCustomizations().add(0, customization);
			}
		}
		final List<Column> notHiddenColumns = new ArrayList<Column>();
		...
		for (Column c : this.table.getColumns()) {
			...
			if (c instanceof FeatureColumn) {
				...
				isVisible = customManager
						.getCustomValueOf(typedElement,
								contentPropH.getIsVisible(), Boolean.class)
						.booleanValue();
	...
	protected static void setMustAskThUserToPutOnTheTopTheLocalCustomizationNextTime(
			final boolean askUserNextTime, final boolean putOnTheTop) {
		PreferenceConstants
				.setAskToPutOnTheTopTheLocalCustomization(askUserNextTime);
		if (!askUserNextTime) { // we doesn't display the message the
								// next time
	...
	public void debugNbColumns() {
		...
		final int nbVisibleColumns = getVisibleColumns(false).size();
		...
		DebugUtils.debug("nbColumns=", String.valueOf(nbColumns)); //$NON-NLS-1$
		DebugUtils.debug("nbVisibleColumnsUsingTheLayer=", //$NON-NLS-1$
				String.valueOf(nbVisibleColumns));
		DebugUtils.debug("nbNonFacetColumns=", //$NON-NLS-1$
				String.valueOf(nbNonFacetColumns));
	}
