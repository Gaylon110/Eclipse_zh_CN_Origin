...
public class ProlificWriterOverlay implements IJavaQuery2<Writer, IImage> {

	private static final String IMG_PATH = "icons/prolificWriter.png"; //$NON-NLS-1$
	private static Image image;

	private static Image getImage() {
		if (ProlificWriterOverlay.image == null) {
			ProlificWriterOverlay.image = ImageDescriptor.createFromURL(
					FileLocator.find(Activator.getDefault().getBundle(),
							new Path(ProlificWriterOverlay.IMG_PATH), null)).createImage();
		}
		return ProlificWriterOverlay.image;
	}
	...
	public IImage evaluate(final Writer writer, final IParameterValueList2 parameterValues, final IFacetManager facetManager)
		...
		IImage result = null;
		if (new IsProlific().evaluate(writer, null, facetManager).booleanValue()) {
			result = ImageUtils.wrap(ProlificWriterOverlay.getImage());
		}
		return result;
	}
}
