...
public class WriterImage implements IJavaQuery2<Writer, IImage> {

	private static final String IMG_PATH = "icons/writer.png"; //$NON-NLS-1$
	private static Image image;

	private static Image getImage() {
		if (WriterImage.image == null) {
			WriterImage.image = ImageDescriptor.createFromURL(
					FileLocator.find(Activator.getDefault().getBundle(),
							new Path(WriterImage.IMG_PATH), null)).createImage();
		}
		return WriterImage.image;
	}
	...
	public IImage evaluate(final Writer writer, final IParameterValueList2 parameterValues, final IFacetManager facetManager)
		...
		return ImageUtils.wrap(WriterImage.getImage());
	}
}
